...
class EVFlowNet(nn.Module):
    def __init__(self, args):
        ...
        if self._args.consider_difference == True:
            image_num = 2
        else:
            image_num = 1

        self.encoder1 = general_conv2d(in_channels = 4*image_num, out_channels=_BASE_CHANNELS, do_batch_norm=not self._args.no_batch_norm)
